...
| **Результат** | `python local_eval.py` (seed 42): **PASS**, чистый результат **4 814 532** (+3,196% к baseline 150 641 084), ROI 49,15, риск 0,0%. `--runs 10`: медиана **4 418 069**, в плюс **10 из 10**. Стресс-миры: все прогоны положительные. |
...
| `pytest` | модульные и интеграционные тесты | ~360 тестов проходят |
...
Все прогоны во всех мирах положительные (100%). Средняя доля от оракула по 9 мирам — около 0,63. Самые большие
разрывы в мирах, где история систематически врёт (`permute_targets`, `flip_sign_30`): агент остаётся в плюсе,
но 18–20 пилотов не хватает, чтобы найти все выгодные цели. `tools/loss_breakdown.py` раскладывает разрыв
